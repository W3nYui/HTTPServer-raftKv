...
raftServerRpcUtil::raftServerRpcUtil(std::string ip, short port) {
  ...
  //*********************************************  */
  // 发送rpc设置
  channel = new MprpcChannel(ip, port, false);
  stub = new raftKVRpcProctoc::kvServerRpc_Stub(channel);
}
...
void raftServerRpcUtil::SetTimeout(std::chrono::milliseconds timeout) { channel->SetTimeout(timeout); }
